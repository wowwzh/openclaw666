...
// [2026-02-27] 修复 Hand Agent 找不到 openclaw 命令的问题
// 在 Windows 上自动设置 OPENCLAW_BIN 环境变量
if (IS_WIN && !process.env.OPENCLAW_BIN) {
    const homedir = process.env.USERPROFILE || process.env.HOME || os.homedir();
    const npmPath = path.join(homedir, 'AppData', 'Roaming', 'npm', 'openclaw.cmd');
    if (fs.existsSync(npmPath)) {
        process.env.OPENCLAW_BIN = npmPath;
    }
}
                                            ...
                                            for (let handAttempt = 1; handAttempt <= HAND_MAX_RETRIES_PER_CYCLE && !handSucceeded; handAttempt++) {
                                                ...
                                                await new Promise((resolveHand, rejectHand) => {
                                                    ...
                                                    const handChild = spawn(openclawBin, finalArgs, {
                                                        ...
                                                        stdio: ['ignore', 'pipe', 'pipe'],
                                                        windowsHide: true,
                                                        shell: IS_WIN  // [FIX] Windows需要shell:true
                                                    });
