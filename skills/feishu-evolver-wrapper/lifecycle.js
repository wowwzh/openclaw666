...
function startDaemon() {
    ...
    // OPENCLAW_BIN 环境变量，解决 Hand Agent 找不到命令的问题
    const npmPath = path.join(process.env.APPDATA || '', 'npm', 'openclaw.cmd');
    const env = { 
        ...process.env, 
        OPENCLAW_BIN: npmPath 
    };

    // 使用 PowerShell Start-Process 启动，保持后台运行
    const psCmd = `Start-Process -FilePath "node" -ArgumentList '\\"${__filename}\\"', 'daemon-loop' -WindowStyle Hidden -PassThru | Select-Object -ExpandProperty Id`;
    ...
    child.unref();
    console.log(`[Daemon] Started internal watchdog daemon via PowerShell`);
}
