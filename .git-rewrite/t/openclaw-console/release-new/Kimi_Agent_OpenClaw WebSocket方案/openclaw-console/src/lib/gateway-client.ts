...
import { getDeviceKeyManager, DeviceCryptoError } from './device-crypto';
...
export class GatewayClient extends EventEmitter<GatewayClientEvents> {
  ...
  private async sendConnectRequest(
    ...
    try {
      // 准备 device identity (如果收到 challenge)
      ...
      if (this.challengeNonce) {
        try {
          ...
          deviceIdentity = await deviceKeyManager.createDeviceIdentity(this.challengeNonce);
          console.log('[GatewayClient] Device identity created:', deviceIdentity.id);
        } catch (cryptoError) {
          console.warn('[GatewayClient] Failed to create device identity:', cryptoError);
          // 继续连接，但不提供设备签名 (依赖纯 Token 认证)
          deviceIdentity = {
